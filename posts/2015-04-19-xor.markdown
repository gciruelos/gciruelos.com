...
In this post we will talk about xor. Xor is a logical operator that outputs  *true* when the two input values are different, and *false* otherwise. It is usually symbolized with \\(\\oplus\\).
...
All of these are really simple to prove. Note that these properties imply that \\((\\mathbb{N}, \\oplus)\\) is an abelian group. More on this later.
...
As \\(\\oplus\\) is commutative and associative, we can group every two elements that are equal together, so we end up with
...
Using the same properties we used over and over again throughout this post, we can design a cipher to encode messages.
